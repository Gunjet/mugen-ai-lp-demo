      ...
      <div className="text-primary-text max-h-[24rem] w-full space-y-4 overflow-auto p-6">
        <div className="flex flex-wrap items-center justify-between">
          ...
          <div className="flex items-center">
            <Button
              className="cursor-pointer rounded-full px-4 text-xs"
              size="sm"
        ...
        <div className="mb-4 flex flex-col items-start space-y-2 xl:flex-row xl:items-center xl:space-y-0 xl:space-x-4">
            ...
            <div className="mb-2 flex items-end">
              ...
              typeof ownCategoryScores.総括 === "number" ? (
                (() => {
                  ...
                  if (
                    urlCategoryScores &&
                    urlCategoryScores.平均 &&
                    typeof urlCategoryScores.平均 === "object"
                  ) {
                    const values = Object.values(urlCategoryScores.平均).filter(
                      (v) => typeof v === "number",
                    ) as number[];
                    if (values.length > 0) {
                  ...
                  return (
                    <>
                      <span
                        className={`text-5xl leading-none font-bold ${isLower ? "text-[#DA2A2A]" : "text-primary-text"}`}
                      >
                        {intPart}
                      </span>
                      {decPart !== undefined && (
                        <span
                          className={`text-3xl leading-none font-bold ${isLower ? "text-[#DA2A2A]" : "text-primary-text"}`}
                        >
                          .{decPart}
                        </span>
                      )}
            ...
            <div className="flex items-end">
              {(() => {
                    ...
                    ? urlMapping
                        ...
                        .filter(
                          (item): item is { url: string } =>
                            ...
                            scoreTotal &&
                            typeof scoreTotal[item.url] === "number",
                        )
                ...
                const ownScore =
                  urlMapping &&
                  urlMapping[0] &&
                  typeof urlMapping[0].url === "string" &&
                  scoreTotal
                    ? scoreTotal[urlMapping[0].url]
                    : undefined;
                const allScores =
                  typeof ownScore === "number"
                    ? [ownScore, ...competitorScores]
                    : competitorScores;
                const avgAll =
                  allScores.length > 0
                    ? (
                ...
                return (
                  <>
                    <span className="text-primary-text !text-[24px] leading-none font-bold">
                      {intPart}
                    </span>
                    {decPart !== undefined && (
                      <span className="text-primary-text !text-[14px] leading-none font-bold">
                        .{decPart}
                      </span>
                    )}
                  </>
          ...
          <div className="flex items-center space-x-31 xl:space-x-2">
            <div className="!text-[10px] font-medium whitespace-nowrap text-[#212121]">
              評価
            </div>
            <div className="flex items-center gap-2">
              <div className="!text-[20px]">
                {gradeConversion(Number(ownCategoryScores?.総括) ?? 0)}
              </div>
        ...
        <div className="flex flex-col space-y-6">
          <div className="text-sm font-medium">
            ...
            {(() => {
              if (
                ...
                return (
                  <span className="text-[#DA2A2A]">
                    ［FV/CTA/コンテンツ/クリエイティブ/アクセシビリティ］を見直すと良い。
                  </span>
                );
              }
              ...
              const categories: CategoryKey[] = Object.keys(
                categoryLabels,
              ) as CategoryKey[];
              ...
              const lowerCategories = categories.filter((cat) => {
                ...
                const avg =
                  urlCategoryScores.平均[
                    cat as keyof typeof urlCategoryScores.平均
                  ];
                return (
                  typeof own === "number" &&
                  typeof avg === "number" &&
              ...
              if (allAboveAvg) {
                ...
                return minCategory ? (
                  <>
                    ...
                    <span className="text-[#DA2A2A]">
                      {categoryLabels[minCategory]}は改善の余地あり。
                    </span>
                  </>
                ) : (
                  <span className="text-[#DA2A2A]">
          ...
          <div>
            <div className="grid grid-cols-3 gap-x-4 gap-y-1">
              ...
              <div className="text-sm-2 font-bold whitespace-nowrap">
                クリエイティブ:
              </div>
              <div className="col-span-2 flex items-center">
                <div className="w-8">
                ...
                <div className="w-8">
                  <span
                    className={`text-[14px] font-bold ${
                      ...
                      typeof urlCategoryScores?.平均?.ACCESSIBILITY ===
                        "number" &&
                      ownCategoryScores.ACCESSIBILITY <
                        urlCategoryScores.平均.ACCESSIBILITY
                        ? "text-red-600"
                        : ""
                    }`}
                  >
