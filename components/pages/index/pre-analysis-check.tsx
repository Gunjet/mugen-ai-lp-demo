...
export default function PreAnalysisCheck() {
  ...
  const [localUpdatedCompetitionUrls, setLocalUpdatedCompetitionUrls] =
    useState<string[]>([]);
  const {
    setDownloadData,
    setScoreDict,
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(competitionUrls) !==
        JSON.stringify(localCompetitionUrls) ||
      competitionUrls.length !== localCompetitionUrls.length
    ) {
      setIsFieldsChanged(true);
    } else {
      setIsFieldsChanged(false);
  ...
  if (Array.isArray(tempImages) && typeof tempImages[2] === "object") {
    imagesForScore = tempImages[2] as {
      [url: string]: { img: string; name?: string };
    };
  }
    ...
    <div className="mt-10 flex flex-col space-y-8">
      <div className="flex items-center space-x-4">
        ...
        <div className="text-secondary-text text-sm">
          ※
          差し替えたい時は上部からURLを変更し、「確認する」を再度実行してください。
        </div>
      </div>
        ...
        >
          ...
          <TableBody>
            <TableRow className="h-16 min-h-16">
              <TableCell className="border-non-color sticky left-0 z-10 border-r border-b bg-[#F8FBFD] p-4 font-medium">
                分析対象
              </TableCell>
              ...
              {competitionUrls.map((url, index) => (
                <TableCell
                  ...
                  className={cn(
                    ...
                    localUpdatedCompetitionUrls.includes(url)
                      ? "bg-corporate/5"
                      : "",
                  )}
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        setLocalUpdatedCompetitionUrls([
                          ...localUpdatedCompetitionUrls,
                          url,
                        ]);
                      } else {
                        setLocalUpdatedCompetitionUrls(
                          localUpdatedCompetitionUrls.filter(
                            (item) => item !== url,
                          ),
                        );
                      }
            ...
            <TableRow>
              ...
              <TableCell className="border-non-color sticky left-[250px] z-10 border-r-4 border-b border-double bg-[#F8FBFD] p-4">
                <div className="border-non-color mx-auto max-h-[253px] w-[129px] overflow-hidden border">
                  <Image
                    src={
                      typeof response[ownUrl] === "string" &&
                      response[ownUrl].startsWith("/dummy")
                        ? response[ownUrl]
      ...
      {isFieldsChanged && (
        <div className="mx-auto mt-1 flex items-center space-x-1 text-xs text-red-500">
          ...
          />
          <span className="text-primary-text text-[13px] leading-none font-medium">
            分析前実行からやり直してください
          </span>
        </div>
      )}
