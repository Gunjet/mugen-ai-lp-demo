...
export default function PreAnalysisInput() {
  ...
  const schema = z
    ...
    .superRefine((data, ctx) => {
      ...
      data.url.forEach((value, i) =>
        combined.push({ type: "url", value: value.trim(), index: i }),
      );
      data.nonRequiredUrl.forEach((value, i) =>
        combined.push({
          type: "nonRequiredUrl",
          value: value.trim(),
          index: i,
      ...
      const byValue = new Map<
        string,
        { type: "own" | "url" | "nonRequiredUrl"; index: number }[]
      >();
      ...
      const totalCompetitorUrls =
        (data.url?.filter((u) => u && u.trim() !== "").length ?? 0) +
        (data.nonRequiredUrl?.filter((u) => u && u.trim() !== "").length ?? 0);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col space-y-4">
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-5 gap-8">
            <div />
            <div className="text-secondary-text col-span-4 mt-2 text-sm">
              ※ URLが読み込めない時に利用ください
            </div>
          </div>
        </div>
