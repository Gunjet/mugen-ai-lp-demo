...
import {
  getRefreshMoment,
  getThemeByMoment,
} from "@/services/gradio-with-dummy";
...
export default function RefreshMomentsPage() {
  ...
  const { commonDict, scoreDict, swotData75, swotData78, setMomentResult } =
    useResultStore();
  const [defaultMoments, setDefaultMoments] = useState<
  ...
  const [combined, setCombined] = useState<{ text: string; value: boolean }[]>([
    ...defaultMoments,
    ...moments,
    ...freeInputs,
  ]);
  ...
  const onSubmit = async () => {
    ...
    const selectedMoments = filtered.map((moment) =>
      Array.isArray(moment.text) ? moment.text : [moment.text],
    );
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  <Checkbox
                    ...
                    disabled={
                      combined.filter((moment) => moment.value).length >= 3 &&
                      !moment.value
                    }
                  />
                  <div>{moment.text}</div>
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent className="w-[190px] max-w-[200px] py-2 text-center text-sm font-medium">
                          <p>
                            フリー入力する場合は、「名称（その具体的な状態）」の形式で記入しないと改善案の品質が低下します。
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  ...
                  <div className="flex flex-col space-y-1">
                    <div className="flex items-center space-x-4 font-medium">
                      <Checkbox
                        ...
                        disabled={
                          combined.filter((moment) => moment.value).length >=
                            3 && !input.value
                        }
                      />
                      <div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="ml-9">
                        {!input.text && input.value && (
                          <div className="mt-1 text-xs text-[#DA2A2A]">
                            入力が必要です。
                          </div>
                        )}
                      </div>
                      <div className="text-secondary-text self-end text-xs font-medium">
                        {input.text.length}/50文字
                      </div>
                    </div>
                  </div>
                </div>
